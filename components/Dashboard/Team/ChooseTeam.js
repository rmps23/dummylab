...
const ChooseTeam = () => {
  ...
  let { userTeams, userTeamsLoading, userTeamsError } = FetchUserTeams() || {};
  const params = useParams();

  if (userTeams) {
    userTeams = userTeams.filter((item) => item.id !== params.teamId);
  }
  ...
  if (userTeamsLoading)
    return (
      <div className="bg-zinc-950 p-3 h-12 w-72 flex items-center justify-center rounded-md relative overflow-hidden group float-right pt-5">
        <CircularLoading size={18} color={"text-zinc-600"} />
      </div>
    );
